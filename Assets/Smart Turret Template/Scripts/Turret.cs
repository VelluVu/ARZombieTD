...
public class Turret : MonoBehaviour {
	...
	private void Shooting() {
		...
		if (Physics.Raycast (VFX.muzzle.position, VFX.muzzle.transform.forward, out hit, parameters.radius)) {
            if (CheckTags(hit.collider) == true && parameters.canShoot == true) {
                parameters.canShoot = false;
				Shot ();
                StartCoroutine(ShootDelay());
                hit.collider.GetComponent<EnemyZombie>().TakeDmg(parameters.power, parameters.damageType);
			}
    ...
    {
        yield return new WaitForSeconds(parameters.ShootingDelay);
